...
class Enemy {
    constructor(x, y, type, speed, isScreenSpaceEntity = false, platform = null) { 
        ...
        // Timer para efeito visual de pulso (Game Feel - Rebound Projectile)
        this.pulseTimer = 0;
    ...
    update(deltaTime, player, scrollOffset, boss) {
        ...
        this.pulseTimer += deltaTime; // Atualiza animação de pulso
        
        if (this.type === 'charger') {
            if (!this.hasCharged) {
                this.chargeTimer -= deltaTime;
                
                // Charger fica imóvel enquanto carrega
                
                if (this.chargeTimer <= 0) {
                    this.hasCharged = true;
                    // Calcula o vetor de ataque em direção ao jogador NA TELA
                ...
                // Após carregar, move-se com a velocidade calculada
        ...
        let trailColor;
        if (this.type === 'homing') { trailColor = '#b8930b'; } 
        else if (this.type === 'rebound') { trailColor = '#5DADE2'; } 
        else if (this.type === 'charger') { trailColor = '#2E8B57'; } 
    ...
    draw(ctx, scrollOffset, verticalScrollOffset = 0, isVertical = false) {
        ...
        // --- INDICADOR DO CHARGER (Sincronizado) ---
        // Só aparece quando faltar 0.7 segundos ou menos para o ataque
        const INDICATOR_DURATION = 0.7; // Mesmo valor de PROJECTILE_INDICATOR_DURATION
        ...
        if (this.type === 'charger' && !this.hasCharged && this.chargeTimer <= INDICATOR_DURATION) {
            ...
            // Calcula o progresso de 0 a 1 durante os 0.7 segundos finais
            const progress = 1 - (this.chargeTimer / INDICATOR_DURATION);
            
            // Animação idêntica ao indicador de spawn (Cresce e desaparece)
            ...
            ctx.beginPath();
            ctx.arc(centerX, centerY, radius * 1.5, 0, Math.PI * 2); // Levemente maior que o padrão para visibilidade
            ctx.fillStyle = 'rgba(46, 139, 87, 0.7)'; // Verde
            ctx.fill();
            ctx.restore();
        }

        // --- INDICADOR DO REBOUND (Onda de choque pulsante) ---
        if (this.type === 'rebound') {
            ...
            ctx.save();
            ctx.globalAlpha = 1.0 - pulseProgress; // Vai ficando transparente
            ctx.beginPath();
        ...
        if (this.type === 'homing') { 
            ...
            // As cores são definidas na lógica de desenho abaixo
        } else { 
